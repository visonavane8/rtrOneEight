...
GLfloat posTR = 2.0f;
GLfloat posCRx = -2.0f , posCRy = -2.2f;
GLfloat posLN = 2.0f; //x should be 0
GLint isDoneT = 0, isDoneC = 0 , isDoneL = 0 ,isDone = 0;
...
void display(void)
{
		...
		GLfloat a, b, c;    // Length  of BC AC AB
		...
		if (isDoneT == 0 && posTR <= 0.0f)
			isDoneT = 1;

		if (isDoneC == 0 && (posCRx >= incenterx ))
		{
			isDoneC = 1;
			posCRx = incenterx; posCRy = incentery;
		}
		if ((isDoneL == 0 && posLN <= 0.0f))
			isDoneL = 1;
		...
		if (isDoneC == 1 && isDoneT == 1 && isDoneL != 1)
		{
			posLN = posLN - 0.001f;
		}
		if (isDoneT == 1 && isDoneC != 1)
		{
			posCRx = posCRx + 0.001f;
			posCRy = posCRy + 0.001f;
		}
		...
		glLoadIdentity();
		glTranslatef(posTR , (-posTR)  , -2.5f);
		glRotatef(angle, 0.0f, 1.0f, 0.0f);
		drawTriangles();

		glLoadIdentity();
		glTranslatef(posCRx, posCRy, -2.5f);
		//glTranslatef(incenterx, incentery, -2.5f);
		glRotatef(angle, 1.0f, 0.0f, 0.0f);
		drawCircle(inradius);

		glLoadIdentity();
		glTranslatef(0.0f, posLN, -2.5f);
