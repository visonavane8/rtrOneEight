...
void uninitialize();
void kundali();
void drawTriangles(GLfloat);
void drawRect(GLfloat);
...
void display(void)
{
	...
	for (i = 0.0f, i1 = 0; i < 1.0f; i += 0.1f, i1++)
	{
		if (i1 > 5)
			i1 = 1;
		drawCircle(i, i1);
...
void drawTriangles(GLfloat i)
{
	glBegin(GL_LINE_LOOP);
	glVertex3f(0.0f,0.5f - i,0.0f);
	glVertex3f(-0.5f + i ,-0.5f + i,0.0f);
	glVertex3f(0.5f - i,-0.5f + i ,0.0f);
	glEnd();
}

void drawRect(GLfloat i)
{
	glBegin(GL_LINE_LOOP);
	glColor3f(1.0f, 1.0f, 1.0f);
	glVertex3f(-0.5f + i , 0.5f - i, 0.0f);
...
void drawCircle(GLfloat i , int i1)
{
	glBegin(GL_LINE_LOOP);
	...
	for (GLfloat angle = 0.0f; angle < 2.0f * M_PI; angle += 0.10f)
	{
